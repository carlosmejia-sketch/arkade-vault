...
import { fetchRecentScores, fetchTopScoresAllGames } from "@/lib/scores";
...
export default async function Home() {
  ...
  const [recentScores, topScores] = await Promise.all([
    fetchRecentScores(supabase, 7),
    fetchTopScoresAllGames(supabase, 5),
  ]);

  const ticker = recentScores.map((row) => ({
    p: row.playerName,
    g: getGame(row.gameId)?.title ?? row.gameId,
    c: getGame(row.gameId)?.color ?? "cyan",
    s: row.score,
    t: tiempoRelativo(row.createdAt),
  }));

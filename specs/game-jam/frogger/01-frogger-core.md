...
- **Nota de implementación (desviación acordada con el usuario durante `/spec-impl-game`):** el spec original describía `components/games/FroggerGame.tsx` (componente React con props) + ruta dedicada `app/games/frogger/play/page.tsx` + tipos `GameRow`/`ScoreRow` de `lib/supabase/types.ts`. Ninguno existe en este repo. Se implementó en su lugar el patrón real vigente (ver CLAUDE.md/mapa-integracion): `lib/games/frogger/engine.ts` (factory `(canvas, callbacks) => Engine`, sin React) registrado en `lib/games/registry.ts`, montado por `components/game-player.tsx` genérico sobre la ruta `app/juegos/[id]/jugar`. La mecánica de juego (grid 16×14, saltos discretos, carriles, vidas, puntuación, temporizador) se conserva igual a lo especificado. También: el Paso 1 (INSERT en tabla `games` de Supabase) no aplica — no existe tal tabla; se agregó la entrada `frogger` directamente al array `GAMES` de `lib/games.ts`, igual que los otros 4 juegos. El color `lime` del data model no existe en `GameColor`; se usó `green` (existente) para no ampliar ese tipo compartido por una sola variante cromática.
